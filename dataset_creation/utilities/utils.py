...
def get_links_media(media, tweets_path):
    ...
    '''
    # Set paths
    csv_path = "{}/Classified/classified_{}.csv".format(tweets_path, media)
    json_path = "{}/extended_json/{}.json".format(tweets_path, media)
...
def extract_information(path, media, tweet_id, soup, debug=False):
    ...
    elif media == 'sextaNoticias':
        if debug: print(tweet_id, "entered in store_news")
        news_author, news_provider, news_creation_datetime, news_uri, \
        news_title, news_headline, news_guid, news_article = obtain_information_sextaNoticias(tweet_id, soup)
